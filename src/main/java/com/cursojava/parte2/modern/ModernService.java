...
public class ModernService {
    public Outcome process(Pedido pedido) {
        String requestId = "REQ-" + UUID.randomUUID().toString().substring(0,
                8);
        try {
            String message = ScopedValue.where(ModernRequestContext.REQUEST_ID,
                    requestId).call(() -> {
                        try (var scope = new StructuredTaskScope.ShutdownOnFailure()) {
                            ...
                            scope.join();
                            scope.throwIfFailed();
                            return "MODERN requestId=" +
                                    ModernRequestContext.REQUEST_ID.get()
                                    + " | " + pago.get()
                                    + " | " + stock.get()
                                    + " | " + envio.get();
                        }
            ...
            return new Outcome.Fail(e.getClass().getSimpleName() + ": " +
                    e.getMessage());
        }
    }
